...
def train_collate(batch):
    ...
    values = torch.stack(transposed_batch[3], 0)
    return images, points, st_sizes, values
...
class RegTrainer(Trainer):
    def setup(self):
        ...
        self.model = MultiEncoderDecoder(args=args)
    ...
    def train_eopch(self):
        epoch_loss_rgb = AverageMeter()
        epoch_loss_t = AverageMeter()
        epoch_loss_fuse = AverageMeter()
        ...
        # Iterate over data.
        for step, (inputs, points, st_sizes, values) in enumerate(self.dataloaders['train']):
            ...
            with torch.set_grad_enabled(True):
                outputs_rgb, outputs_t, outputs_fuse, w_rgb, w_t, w_illumination = self.model(inputs, values)
                prob_list = self.post_prob(points, st_sizes)
                loss_rgb = self.criterion(prob_list, outputs_rgb)
                loss_t = self.criterion(prob_list, outputs_t)
                loss_fuse = self.criterion(prob_list, outputs_fuse)
                w = torch.multiply(w_illumination, w_rgb) + w_t
                loss_normalize = nn.L1Loss()(w, torch.ones_like(w))
                loss = loss_rgb + loss_t + loss_fuse + args.w_normalize * loss_normalize
                ...
                if type(inputs) == list:
                    ...
                    N = inputs.size(0)
                pre_count = torch.sum(outputs_fuse.view(N, -1), dim=1).detach().cpu().numpy()
                res = pre_count - gd_count
                epoch_loss_rgb.update(loss_rgb.item(), N)
                epoch_loss_t.update(loss_t.item(), N)
                epoch_loss_fuse.update(loss_fuse.item(), N)
                epoch_mse.update(np.mean(res * res), N)
                epoch_game.update(np.mean(abs(res)), N)
                
            if step % 200 == 0:
                print(('Epoch: [{0}][{1}/{2}]\tLoss_rgb {3}\tLoss_t {4}\tLoss_fuse {5}\tLoss_normalize {6}\t')
                    .format(self.epoch, step, len(self.dataloaders['train']), loss_rgb.item(), loss_t.item(), loss_fuse.item(), loss_normalize.item()))

        logging.info('Epoch {} Train, Loss_rgb: {:.2f}, Loss_t: {:.2f}, Loss_fuse: {:.2f}, GAME0: {:.2f} MSE: {:.2f}, Cost {:.1f} sec'
                     .format(self.epoch, epoch_loss_rgb.get_avg(), epoch_loss_t.get_avg(), epoch_loss_fuse.get_avg(), \
    ...
    def val_epoch(self):
        ...
        for inputs, target, name, value in self.dataloaders['val']:
            if type(inputs) == list:
                inputs[0] = inputs[0].to(self.device)
                inputs[1] = inputs[1].to(self.device)
            else:
            ...
            with torch.set_grad_enabled(False):
                _, _, outputs, _, _, _ = self.model(inputs, value)
    ...
    def test_epoch(self):
        ...
        for inputs, target, name, value in self.dataloaders['test']:
            if type(inputs) == list:
                inputs[0] = inputs[0].to(self.device)
                inputs[1] = inputs[1].to(self.device)
            else:
